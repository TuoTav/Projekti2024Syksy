...
int my_lbs_send_sensor_notify(uint32_t sensor_value_x,uint32_t sensor_value_y,uint32_t sensor_value_z,uint32_t pos)
{
	if (!notify_mysensor_enabled) {
		return -EACCES;
	}
	char data[128];
    snprintf(data, sizeof(data), "%u,%u,%u,%u", sensor_value_x, sensor_value_y, sensor_value_z,pos);
