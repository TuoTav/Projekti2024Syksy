...
static uint32_t sensor_x=1;
static uint32_t sensor_y=2;
static uint32_t sensor_z=3;
...
{
	printk("Send data thread started\n");
	...
	while (1) {
		...
		my_lbs_send_sensor_notify(sensor_x,sensor_y,sensor_z,position);
